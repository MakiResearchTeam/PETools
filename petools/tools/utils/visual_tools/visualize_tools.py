...
def draw_skeleton(
        ...
        draw_pose_name: bool = False, pose_name_position: tuple = (100, 100), pose_name_text_size: int = 2,
        draw_pose_conf: bool = False, pose_conf_position: tuple = (120, 120), pose_conf_text_size: int = 2,
    ...
    for indx in range(len(humans)):
        ...
        if draw_pose_name and pose_name_list is not None:
            cv2.putText(
                image, str(pose_name_list[indx]), pose_name_position, cv2.FONT_HERSHEY_SIMPLEX, pose_name_text_size, (0, 0, 255), 6
            )

        if draw_pose_conf and pose_conf_class_list is not None:
            cv2.putText(
                image, str(pose_conf_class_list[indx]), pose_conf_position, cv2.FONT_HERSHEY_SIMPLEX, pose_conf_text_size, (0, 0, 255), 6
            )
        ...
        if draw_pose_angle and pose_angle_list is not None:
            ...
            for single_angle in pose_angle_list[indx]:
                cv2.putText(
                    image, str(single_angle), (start_x, start_y), cv2.FONT_HERSHEY_SIMPLEX, pose_angle_text_size, (0, 0, 255), 6
                )
